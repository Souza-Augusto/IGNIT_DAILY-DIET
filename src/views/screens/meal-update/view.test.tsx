...
import { MealUpdateProps, useMealUpdateViewModel } from './view-model';
import { MealUpdate } from './view';
...
const CREATE_MOCK_VIEW_MODEL = ({
  ...
  name,
  hour,
}: CreateMockViewModelProps): MealUpdateProps => ({
  fetchData: MOCK_FETCH_DATA,
  date,
  description,
  dialogMessage,
  dialogVisible,
  handleMealUpdate: MOCK_HANDLE_REGISTER_MEAL,
  healthy,
  hour,
  isLoading,
  name,
...
function makeSUT({
  ...
  jest.mocked(useMealUpdateViewModel).mockReturnValue(MOCK_VIEW_MODEL);

  const SUT = render(<MealUpdate />);

  return SUT;
}

describe('screen:update-meal view', () => {
  ...
  it('should OffDietButton render correctly', () => {
    ...
    const TITLE = getByText('Salvar alterações');
